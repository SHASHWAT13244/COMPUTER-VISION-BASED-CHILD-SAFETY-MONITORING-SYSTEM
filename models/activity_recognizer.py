...
class ActivityRecognizer:
    ...
    def predict_activity(self, keypoint_sequence, return_probabilities=False):
        ...
        Returns:
            activity: Predicted activity label (or None)
            confidence: Confidence score
            (optional) probabilities: Probability distribution
        """
        if keypoint_sequence is None or len(keypoint_sequence) < self.sequence_length:
        ...
        with torch.no_grad():
            ...
            activity = (self.activity_labels[pred_idx]
                        if pred_idx < len(self.activity_labels) else 'unknown')
    ...
    def predict_with_memory(self, keypoints, threshold=0.6):
        ...
        if len(self.sequence_buffer) >= self.sequence_length:
            ...
            if activity is not None and confidence >= threshold:
                return activity, confidence
    ...
    def get_activity_distribution(self, keypoint_sequence):
        ...
        activity, confidence, probs = self.predict_activity(
            keypoint_sequence, return_probabilities=True
        )
        return probs
